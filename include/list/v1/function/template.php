...
function get_listing_template_output($structure, & $listing, & $key, & $translation, $load, $display, $part, $login_user_id, & $style = null, $type = '') {
	...
	if (preg_match('/^todo/', $v1)) {
		...
		case 'channel_parent_id':
			$grab .= tt('element', 'channel_parent_id') . ': ' . to_html($listing[$v1]);
		break;
		case 'carry_value':
		...
		case 'more_toggle':
			...
			$s2 = '';
			 if ($load == 'view')
				$s2 = 'view_';
			$grab .= '<nobr><span class="spacer">&gt;&gt;</span><a id="' . $s2 . to_html($s1) . '_id_' . (int)$listing[$s1 . '_id'] . '_toggle" href="javascript: more_toggle(\'' . $s2 . to_html($s1) . '_id_' . (int)$listing[$s1 . '_id'] . '\');">' 
		...
		case 'way_name':
			...
			if ($listing['source_user_id'] == $login_user_id)
				$grab .= '<span' . get_color_style($color['direction_name']) . ' class="direction_name">=)</span>';
			else
				$grab .= '<span' . get_color_style($color['direction_name']) . ' class="direction_name">(=</span>';
			$grab .= ' ';
		break;
		...
		case 'team_owner':
			...
			if ($i1 == 1) {
				$grab .= '<a' .
					get_color_style($color['link']) . ' href="' . $href_prepend . 'contact_view/'
			...
			if ($i2 == 1) {
				$grab .= ' <a' . get_color_style($color['link']) .  ' href="' . $href_prepend . 'user_view/'
					. to_html(ff('list_name=&list_type=&lock_user_id=' . (int)$listing['team_owner_user_id'] . $glue . $href_append, $ff_level))
					. '"><span' . get_color_style($color['user_name']) . ' class="user_name">'
		...
		case 'status_name':
			$grab .= '<span' .
				get_color_style($color['status_name']) .
				' class="' . $v1 .
			'">' .  tt(str_replace('_name', '', $v1), $listing[$v1]) . '</span>';
		break;
		case 'description':
		...
		case 'offer_name':
			$grab .= '<span' .
				get_color_style($color['offer_name']) .
				' class="' . $v1 .
			'">' .
				to_html($listing[$v1]) .
			'</span>';
		...
		case 'tag_name':
			$grab .= '<span' .
				get_color_style($color['thing_name']) .
				' class="' . $v1 .
			'">' .
				#. tt('tag', $listing[$v1])
		...
		case 'team_name': 
			$grab .= '<span' .
				get_color_style($color['team_name']) . 
				' class="' . $v1 .
			'">' . to_html($listing[$v1]) . '</span>';
		break;
		case 'group_name': 
			# group_list and groupmate_list not completely stripped out of ts yet
			$grab .= '<span' .
				get_color_style($color['group_name']) .
				' class="' . $v1 .
			'">' . to_html($listing[$v1]) . '</span>';
		break;
	}
