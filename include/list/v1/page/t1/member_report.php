...
<div class="doc_box">
	...
	<dl><?
		...
		if (is_numeric($k1)) { # $k1 = 'cycle_restart' is getting set out of nowhere (on 1 server) ?> 
			<dt><a href="./cycle_report/<?= ff('channel_parent_id=' . (int)$k1); ?>"><?= to_html($data['member_report']['channel'][$k1]['info']['name']); ?></a> - <?= $data['member_report']['payout_order'][$k1] ? (int)$data['member_report']['payout_order'][$k1] : 0; ?> &rarr; <?= (int)$data['member_report']['order'][$k1]; ?></dt> 
